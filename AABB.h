...
struct AABB{
    ...
    static AABB join(const AABB& lhs, const AABB& rhs) {
        // return an AABB object that contains given two AABB objects
        int minx = getMin(lhs.minX, rhs.minX);
        int maxx = getMax(lhs.maxX, rhs.maxX);
        int miny = getMin(lhs.minY, rhs.minY);
        int maxy = getMax(lhs.maxY, rhs.maxY);

        return AABB(minx, miny, maxx, maxy);
    }

    static bool contains(const AABB& bigRect, const AABB& smallRect) {
        // returns true if big rectancle contains small rectangle
        return (bigRect.minX <= smallRect.minX) && (bigRect.minY <= smallRect.minY) && (bigRect.maxX >= smallRect.maxX) && (bigRect.maxY >= smallRect.maxY);
    }
